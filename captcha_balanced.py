...
用法:
    from captcha_balanced import solve_captcha_from_base64
...
def _get_votes(click_area, bbox):
    """12次OCR/区域: 3种padding × (beta原图 + beta放大 + beta反色 + normal原图)"""
    x1, y1, x2, y2 = bbox
    votes = Counter()
    for pad in [3, 5, 8]:
        ...
        r = _ocr_beta.classification(_img_to_bytes(ci_inv))
        if len(r) == 1:
            votes[r] += 1
        # normal 原图
